...
class M2TransformerModel(TransformerModel):
    ...
    def make_model(self, src_vocab, tgt_vocab, N_enc=6, N_dec=6, d_model=512, d_ff=2048, h=8, dropout=0.1):
        ...
        encoder = MemoryAugmentedEncoder(N_enc, 0, attention_module=ScaledDotProductAttentionMemory, attention_module_kwargs={'m': 40})
        # Another implementation is to use MultiLevelEncoder + att_embed
        decoder = MeshedDecoder(tgt_vocab, 54, N_dec, -1) # -1 is padding;
        model = Transformer(0, encoder, decoder, d_model, dropout) # 0 is bos
        return model
    ...
    def __init__(self, opt):
        ...
        # Notes: The dropout in MAEncoder is different from demo att_embed, mine is 0.5?
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        seq = seq.clone()
        seq[~seq_mask.any(-2)] = -1 # Make padding to be -1 (demo dataloader uses 0 as padding)
    ...
    #     seq, logprobs, seqLogprobs = self.model.beam_search(att_feats, self.seq_length, 0, beam_size, return_probs=True, out_size=beam_size)
    #     seq = seq.reshape(-1, *seq.shape[2:])
