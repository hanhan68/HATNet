...
####用于new_GeoRSCLIP_trans4 # 可视化解码器注意力图
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        "Apply residual connection to any sublayer with the same size."
         # sublayer 返回（attn_out, attn_weight）
...
img = Image.open("/home/rzl/project/data/nwpu/imgs/airplane/airplane_677.jpg")
img = trans(img) # (3, 224, 224)
#############################################################################
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        ##########################################################################################################
        attn_last = attns[-1] # (NHLD) # 最后一层的注意力
        attn = attn_last[46, : , -1, :] # 选择第n个图像的刚生成词的注意力
        attn = attn.view(attn.shape[0], 16, 16)
        attentions = nn.functional.interpolate(attn.unsqueeze(0), scale_factor=14, mode="nearest")[0].cpu().numpy() #(H,W,H)
        plt.imshow(img)
        plt.imshow(np.mean(attentions, 0), alpha=0.5, cmap="hot")
        plt.savefig("/home/rzl/桌面/show/{}.png".format(self.count))
        self.count+=1
        ##########################################################################################################
        return self.norm(x)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        _, attn = self.src_attn(x, m, m, src_mask) # 获取注意力 ##########################
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x): # x (16, 512, 196)
...
class TransformerModel(AttModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq) # (16, 512, 196)

        out = self.model(att_feats, seq, att_masks, seq_mask) # dec_cross_attns:(N*5,H,L,D)
