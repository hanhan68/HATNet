...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, dropout):
        ...
        # self.self_attn_spatial = self_attn
        # self.self_attn_channel = MultiHeadedAttention(8, 256, 0.1)
    ...
    def forward(self, x, mask):
        ...
        # x = self.sublayer[0](x, lambda x: self.self_attn_spatial(x, x, x, mask)) # token维度
        # x = self.sublayer[1](x, self.feed_forward[0])
        #
        # x = x.transpose(-1, -2) # 交换维度，dim维度做注意力
        # x = self.sublayer[2](x, lambda x: self.self_attn_channel(x, x, x, mask))  # dim维度
        # x = x.transpose(-1, -2) # 交换回来
        # x = self.sublayer[3](x, self.feed_forward[1])
