...
import sys
sys.path.append('/home/rzl/lab/nwpu-caption/captioning/utils')
sys.path.append("/home/rzl/lab/nwpu-caption/coco-caption/pycocoevalcap/eval.py")
...
def language_eval(dataset, preds, preds_n, eval_kwargs, split):
    ...
    if len(preds_n) > 0:
        ...
        else: dataset_file = "data/dataset.json"
    ...
    while True:
        ...
        if beam_size > 1 and verbose_beam:
            for i in range(fc_feats.shape[0]):
                print('\n'.join([utils.decode_sequence(model.vocab, _['seq'].unsqueeze(0))[0] for _ in model.done_beams[i]])) # 这里进行了beam search
                print('--' * 10)
        sents = utils.decode_sequence(model.vocab, seq) # 编码--->caption
        ...
        #import pdb;pdb.set_trace()  #一个batch后停止
