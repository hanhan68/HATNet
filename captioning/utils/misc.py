...
def get_std_opt(model, optim_func='adam', factor=1, warmup=2000):
    ...
    filt = filter(lambda p: p.requires_grad, model.parameters()) # 加入了bert冻结参数，所以要过滤掉冻结的参数
    return NoamOpt(model.d_model, factor, warmup, optim_func(filt, lr=0, betas=(0.9, 0.98), eps=1e-9))